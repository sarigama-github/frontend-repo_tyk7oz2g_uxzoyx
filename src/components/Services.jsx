...
export default function Services() {
  return (
    <section id="diensten" className="py-20 bg-neutral-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl">
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white">Diensten</h2>
          <p className="mt-3 text-neutral-300">Alles wat je nodig hebt om online te winnen — van vindbaarheid tot verbluffende websites.</p>
        </div>
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((s, i) => (
            <div key={i} className="border border-neutral-800 rounded-2xl p-6 hover:shadow-md hover:border-yellow-400/60 transition bg-neutral-900">
              <s.icon className="h-8 w-8 text-yellow-400" />
              <h3 className="mt-4 font-bold text-white">{s.title}</h3>
              <p className="mt-2 text-sm text-neutral-300">{s.desc}</p>
              <ul className="mt-4 space-y-1 text-sm text-neutral-400 list-disc list-inside">
                {s.bullets.map((b, j) => (
                  <li key={j}>{b}</li>
                ))}
              </ul>
